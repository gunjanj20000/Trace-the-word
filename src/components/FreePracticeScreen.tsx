...
export const FreePracticeScreen: React.FC<FreePracticeProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full w-full bg-gradient-to-b from-purple-50/70 via-white to-sky-100/60 overflow-hidden select-none">
      {/* Top Header */}
      <header className="flex-none flex items-center justify-between px-4 sm:px-8 py-3 pt-[calc(0.75rem+var(--safe-top))] z-20">
        <button
          onClick={onHome}
          aria-label="Back to Home"
          className="w-14 h-14 sm:w-16 sm:h-16 rounded-3xl bg-white shadow-soft border-2 border-purple-100 flex items-center justify-center text-slate-700 active:scale-95 transition hover:bg-purple-50"
        >
          <Home className="w-7 h-7 sm:w-8 sm:h-8 text-purple-600" />
        </button>

        <div className="flex items-center gap-2 bg-white/90 backdrop-blur px-5 py-2.5 rounded-3xl shadow-soft border border-purple-100">
          <Sparkles className="w-6 h-6 text-amber-500" />
          <span className="text-xl sm:text-2xl font-black text-purple-700">Free Practice</span>
          {completedCount > 0 && (
            <span className="ml-2 px-2.5 py-0.5 bg-purple-100 text-purple-800 rounded-full text-sm font-bold">
              ★ {completedCount}
            </span>
          )}
        </div>
        ...
        <button
          ...
          className="w-14 h-14 sm:w-16 sm:h-16 rounded-3xl bg-white shadow-soft border-2 border-purple-100 flex items-center justify-center active:scale-95 transition text-purple-700 hover:bg-purple-50"
        >
            ...
            return (
              <button
                ...
                className={`w-11 h-12 sm:w-12 sm:h-14 rounded-2xl font-extrabold text-xl sm:text-2xl transition-all ${
                  isCurrent
                    ? 'bg-purple-600 text-white shadow-lg ring-4 ring-purple-200 scale-110'
                    : 'bg-white hover:bg-purple-50 text-slate-700 shadow-sm border border-purple-100'
                }`}
              >
                {char}
              </button>
      ...
      <footer className="flex-none flex items-center justify-between px-6 sm:px-12 py-4 pb-[calc(1rem+var(--safe-bottom))] z-20">
        <button
          onClick={handleReset}
          className="h-16 px-6 sm:px-8 rounded-3xl bg-white shadow-soft border-2 border-purple-100 flex items-center gap-2 text-slate-700 font-bold text-lg active:scale-95 transition hover:bg-purple-50"
        >
        ...
        <button
          onClick={handleHearLetter}
          className="h-16 px-8 sm:px-10 rounded-3xl bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xl shadow-lg flex items-center gap-2 active:scale-95 transition"
        >
